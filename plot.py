...
def get_superresolution():
    for batch in test_loader:
        ...
        sr_test_tensor = sr_test.detach().cpu()

    return sr_test_tensor, hr_test_tensor, lr_test_tensor

def get_mse(sr_test_tensor, hr_test_tensor, lr_test_tensor):
...
def get_relativeDistance(sr_test_tensor, hr_test_tensor, lr_test_tensor):
...
sr_test_tensor, hr_test_tensor, lr_test_tensor = get_superresolution()
mse_sr_hr, mse_lr_hr = get_mse(sr_test_tensor, hr_test_tensor, lr_test_tensor)
rel_diff_sr, rel_diff_lr = get_relativeDistance(sr_test_tensor, hr_test_tensor, lr_test_tensor)
mse_average = sum(mse_sr_hr) / len(mse_sr_hr)
...
#1D representation
def plot_1D(x_lr, x_hr, x_sr, y_lr, y_hr, y_sr, z_lr, z_hr, z_sr):
    time = np.linspace(0, 7, 800)  # Assume this corresponds to the time variable in your dataset

    # Plot for the 'x' dimension
